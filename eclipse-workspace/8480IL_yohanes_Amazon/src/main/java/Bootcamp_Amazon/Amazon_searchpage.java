...
public class Amazon_searchpage {
	WebDriver driver;
@FindBy(xpath="//span[contains(text(),'Introducing Amazon Fire TV 55\" 4-Series 4K UHD sma')]") List <WebElement> list;
...
public void ClickonItem(String item) throws InterruptedException {
	...
	if(Item.get(i).getText().contains("Introducing Amazon Fire TV 55\" 4-Series 4K UHD smart TV")) {
		Thread.sleep(6000);
	list.get(i).click();
	break;
	}else if(Item.get(i).getText().contains("LG 65UP8000PUR Alexa Built-In 65\" 4K Smart UHD TV (2021)")) {
		Thread.sleep(6000);
	list.get(i).click();
	break;
	}
